...
class ControllerSensors : Service()
{
    ...
    private var countGPS = false
    private var threadCheckDisplay = true
    ...
    private fun getSensorsInfo()
    {
        ...
        val locationListener: LocationListener = object : LocationListener
        {
            ...
            override fun onProviderEnabled(provider: String) { countGPS = true }
            override fun onProviderDisabled(provider: String) { countGPS = false; setZeroTimeGPS() }
        }
        ...
        Executors.newSingleThreadScheduledExecutor().scheduleAtFixedRate(
       {
           ...
           this.timeWorkGPS += if (countGPS) TIME_UPDATE else 0
           ...
           else
           {
               ...
               if (this.timeWorkMNetwork != 0) requestsSQL.writeToDB(CHANNEL_MNETWORK, timeWorkMNetwork)
    ...
    {
        Log.i("Sensors info seconds", "|| WIFI Time: $timeWorkWifi" +
                ", MNetwork Time: $timeWorkMNetwork" +
                ", GPS Time: $timeWorkGPS" +
                ", Bluetooth Time: $timeWorkBluetooth" +
                ", Display Time: $timeWorkDisplay ||")
    ...
    private fun notificationAction(infoText: String, channelID: String, btnName: String, notificationID: Int)
    {
        ...
        if (channelID != CHANNEL_DISPLAY)
        {
          builder = NotificationCompat.Builder(this, channelID)
            ...
            .setAutoCancel(true)
            .addAction(R.mipmap.ic_launcher_foreground, btnName, PendingIntent.getActivity(this, 100, listenerIntent, PendingIntent.FLAG_UPDATE_CURRENT))
            .addAction(R.mipmap.ic_launcher_foreground, "Всё впорядке", PendingIntent.getActivity(this, 0, intentCancer, PendingIntent.FLAG_CANCEL_CURRENT))
        ...
        else
        {
            builder = NotificationCompat.Builder(this, channelID)
                ...
                .setStyle(NotificationCompat.BigTextStyle().bigText(infoText))
                .setAutoCancel(true)
                .addAction(R.mipmap.ic_launcher_foreground, btnName, PendingIntent.getActivity(this, 0, intentCancer, PendingIntent.FLAG_CANCEL_CURRENT))
                .setPriority(NotificationCompat.PRIORITY_MAX)
        }
    ...
    private fun screenActivity()
    {
        ...
        if (isScreenOn)
        {
            ...
            for ((key, value) in this.enabledAppInfo.entries)
            {
                ...
                if (value >= FINISH_TIME_DISPLAY && timeWorkDisplay >= FINISH_TIME_DISPLAY)
                {
                    notificationAction(
                    ...
                    displayCheck()
                    this.enabledAppInfo.clear()
                    break
                }
            }
        }
        else { actionDisplayOff(false); this.enabledAppInfo.clear() }
    }
    ...
    private fun actionDisplayOff(displayActive: Boolean)
    {
        ...
        if (timeWorkDisplay > 0) requestsSQL.writeToDB(CHANNEL_DISPLAY, timeWorkDisplay)
        ...
        if (threadCheckDisplay)
        {
            Executors
                ...
                .schedule({
                    ...
                    allNotifyId.clear()
                    threadCheckDisplay = true
                }, 20, TimeUnit.SECONDS)

            threadCheckDisplay = false
        }
    }

    private fun notifyVibrate()
    {
        if (vibrator.hasVibrator())
        {
            ...
            else vibrator.vibrate(VIBRATE_TIME)
        }
    }
    ...
    companion object
    {
        ...
        private const val DEFAULT_FINISH_TIME = 3600000 // For testing, substitute set the control time to a value of 40 seconds.
        private const val VIBRATE_TIME = 3000L

        private const val NOTIFY_GPS = 1
        const val CHANNEL_GPS = "GPS"

        private const val NOTIFY_WIFI = 2
        const val CHANNEL_WIFI = "WiFi"

        private const val NOTIFY_MNETWORK = 3
        const val CHANNEL_MNETWORK = "MobileNetwork"

        private const val NOTIFY_BLUETOOTH = 4
        const val CHANNEL_BLUETOOTH = "Bluetooth"

        private const val NOTIFY_DISPLAY = 5
        const val CHANNEL_DISPLAY = "Display"
    }
}